...
class DateOfLastReportModel {
    static async  findAll(): Promise<DateOfLastReport[] | null> {
        const [dateOfLastReports, metadata] = await db.query ('SELECT BIN_TO_UUID (date_of_last_report_id) AS date_of_last_report_id,day_of_last_report from DateOfLastReport ')
        return dateOfLastReports as DateOfLastReport[];
    }

    static async findById(id: string):Promise<DateOfLastReport | null> {
        const [DateOfLastReport, metadata] = await db.query('SELECT BIN_TO_UUID (date_of_last_report_id) AS date_of_last_report_id,day_of_last_report from DateOfLastReport')
    ...
    static async create(DateOfLastReportData:DateOfLastReport): Promise<DateOfLastReport | null> {
        ...
        const [newDateOfLastReport, metadata] =await db.query('INSERT INTO DataOfLastReport (date_of_last_report_id, day_of_last_report) VALUES (UUID_TO_BIN(UUID()), ?)',
        {
    ...
    static async update(DateOfLastReportData: DateOfLastReport, id: string): Promise<DateOfLastReport | null> {
        try {
            const { date_of_last_report_id, day_of_last_report } = DateOfLastReportData;
            await db.query(
                'UPDATE DateOfLastReport SET day_of_last_report = ? WHERE date_of_last_report_id = UUID_TO_BIN(?)',
                {
            ...
            return updatedDateOfLastReport || null;
              ...
              console.error('Error updating DateOfLastReport:', error);
              throw error; 
            }
        }
    ...
    static async deleteById(id: string): Promise<DateOfLastReport | null> {
        ...
        await db.query('DELETE FROM DateOfLastReport WHERE day_of_last_report = UUID_TO_BIN(?)',
              {
                replacements: [id]
              });
